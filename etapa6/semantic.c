...
void checkUsage(AST* node) {
  ...
  switch (node->type) {
    ...
    // case AST_INPUT: 
    //   if(node->symbol->type != SYMBOL_VAR){
		// 		fprintf(stderr, "Semantic error in line %d: only allowed scalar variables.\n", node->lineNumber);
		// 		errorCounter++;
		// 	}
...
void compareCalledArguments(AST* node, AST* decl) {
  if(node->son[0] != NULL) {
    //debug
    // node->son[0]->dataType?printf("Node->son->datatype = %d", node->son[0]->dataType):printf("Has no datatype");
    // printf("Node: %d", node->dataType);
